...
function PrepareData() {
  ...
  const getFeatureSelectElem = (includedExcludedColumns, finalColumnsStatus, onFinalize) => {
    return {
  ...
  useEffect(() => {
    ...
    if(includedExcludedColumns) {
      newElems.push(getFeatureSelectElem(includedExcludedColumns, finalColumnsStatus, onFinalize));
    } else if (columnsFound.length > 0) {
      const incExcColumns = [
        columnsFound.slice(0,20).map((word) => ({word})),
        columnsFound.slice(20).map((word) => ({word}))
      ]
      newElems.push(getFeatureSelectElem(incExcColumns, finalColumnsStatus, onFinalize));
    }
